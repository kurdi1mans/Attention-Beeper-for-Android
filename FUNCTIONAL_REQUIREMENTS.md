...
Attention Beeper plays a synthesized beep sound at user-configured intervals to help the user maintain focus. It works completely offline. There is no backend; all logic runs on the client.
...
While a session is running, the app displays a live countdown to the next beep in minutes and seconds (e.g. `1:05`, `0:30`). The countdown disappears when the session is stopped.
...
The packages and libraries selection for the Android app must happen very carefully to prevent any chances of the app losing background atomonmous operations (especially when running in random mode which requires recalculation of next beep time after each beep).
